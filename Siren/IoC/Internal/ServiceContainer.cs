...
namespace Siren.IoC.Internal
{
    ...
    internal sealed class ServiceContainer : IServiceProvider, IDisposable
    {
        ...
        /// </summary>
        /// <exception cref="InvalidOperationException">Thrown if the caller is not the Siren assembly.</exception>
        public void Dispose()
        {
            if (!Assembly.GetCallingAssembly().IsExecutingAssembly())
            {
                throw new InvalidOperationException("The service container can only be disposed of by the Siren assembly.");
            }
